...
void CBonusManager::CheckSync(BONUS_INFO* lpInfo) // OK
{
	...
	if(day > 0)
	{
		LogAdd(LOG_BLACK,"[Bonus Manager] (%d) Hora de início da sincronização. [%d] dia [%d] hora [%d] min restantes",lpInfo->Index,day,hr,min);
	}
	else if(hr > 0)
	{
		LogAdd(LOG_BLACK,"[Bonus Manager] (%d) Hora de início da sincronização. [%d] horas [%d] minutos restantes",lpInfo->Index,hr,min);
	}
	else
	{
		LogAdd(LOG_BLACK,"[Bonus Manager] (%d) Hora de início da sincronização. [%d] min restantes",lpInfo->Index,min);
	}
}
