...
{
	LogAdd(LOG_BLACK,"[Castle Deep] Estado definido como DESATIVADO");
}
...
void CCastleDeep::SetState_EMPTY() // OK
{
	...
	LogAdd(LOG_BLACK,"[Castle Deep] Definir estado como VAZIO");
}
...
void CCastleDeep::SetState_START() // OK
{
	...
	LogAdd(LOG_BLACK,"[Castle Deep] Definir estado INICIAR");
}
...
void CCastleDeep::CheckSync() // OK
{
	...
	if(day > 0)
	{
		LogAdd(LOG_BLACK,"[Castle Deep] Tempo de abertura da sincronização. [%d] dia [%d] hora [%d] min restantes",day,hr,min);
	}
	else if(hr > 0)
	{
		LogAdd(LOG_BLACK,"[Castle Deep] Tempo de sincronização aberto] horas] minutos restantes",hr,min);
	}
	else
	{
		LogAdd(LOG_BLACK,"[Castle Deep] Tempo de abertura da sincronização. [%d] min restantes",min);
	}
}
