	...
	if(this->m_StateTimeData.empty() == 0)
	{
		for(size_t n = 0; n < this->m_StateTimeData.size(); n++)
		{
			if((n+1) < this->m_StateTimeData.size())
			{
				...
				if(result < 0)
				{
					LogAdd(LOG_RED,"[Castle Siege] CCastleSiege::LoadData() - A ordem das datas está incorreta (falha na classificação): %d-%d",this->m_StateTimeData[n].State,this->m_StateTimeData[n+1].State);
				}
				else
				{
...
void CCastleSiege::MainProc() // OK
{
	...
	if((GetTickCount()-LastTickCount) > 3600000)
	{
		...
		if(LeftSiegeDate >= 0)
		{
			LogAdd(LOG_BLACK,"[Castle Siege] Hora de início da sincronização. Restam [%02d] dias",(LeftSiegeDate/60/60/24));
		}
	}
}
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Defina o estado como IDLE1.");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado IDLE2");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado SIGN2");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado IDLE3");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado NOTIFICAR");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado como PRONTO");
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado INICIAR");
	...
	if(this->m_SiegeGuildDataMap.empty() == 1)
	{
		...
		this->SetState(CS_STATE_END,1);
		LogAdd(LOG_RED,"[Castle Siege] Definir estado INICIAR - Sem ataque à guilda");
	}
	else
	{
		this->GDCastleSiegeNpcSaveSend();
...
{
	LogAdd(LOG_BLACK,"[Castle Siege] Definir estado FIM");
...
int CCastleSiege::CheckSync() // OK
{
	...
	if(this->m_bFixCastleStateStartDate != 0)
	{
		...
		{
			ErrorMessageBox("[Castle Siege] CCastleSiege::CheckSync() - ESTADO FIXO: %d NÃO ENCONTRADO NA AGENDA (P.S.> Verifique os arquivos 'MuCastleData.dat' e 'commonserver.cfg')",this->m_iFixCastleSpecificState);
			this->m_bFixCastleStateStartDate = 0;
		}
	}
			...
			if((NpcData.Class == 277 || NpcData.Class == 283) && NpcData.aIndex != -1)
			{
				if(gObjIsConnected(NpcData.aIndex) == 0 || (gObj[NpcData.aIndex].Class != 277 && gObj[NpcData.aIndex].Class != 283) || gObj[NpcData.aIndex].Live == 0)
				{
					...
					if(NpcData.Class == 277)
					{
						...
						LogAdd(LOG_BLACK,"[Castle Siege] O Portão do Castelo foi destruído. (Índice:%d, X:%d, Y:%d)", it->Index, it->X, it->Y);
					}
					else if(NpcData.Class == 283)
					{
						LogAdd(LOG_BLACK,"[Castle Siege] A estátua em defesa da propriedade foi destruída. (Índice:%d, X:%d, Y:%d)", it->Index, it->X, it->Y);

						if(this->CheckGuardianStatueExist() == 0)
						{
							this->ClearCastleTowerBarrier();
							this->m_CastleTowerEnable = 1;
							LogAdd(LOG_BLACK,"[Castle Siege] A entrada para a Sala da Coroa está aberta.");
						}
					}
	...
	if(lpCrownAccess->CastleJoinSide >= 2 && lpSwitchUser1->CastleJoinSide >= 2 && lpSwitchUser2->CastleJoinSide >= 2)
	{
		if(lpCrownAccess->CastleInvolved != 0)
		{
			if(lpCrownAccess->CastleJoinSide == lpSwitchUser1->CastleJoinSide && lpCrownAccess->CastleJoinSide == lpSwitchUser2->CastleJoinSide)
			{
				...
				{
					LogAdd(LOG_BLACK,"[Castle Siege] A guilda %s tomou posse do castelo.", lpCrownAccess->GuildName);
...
bool CCastleSiege::CheckCastleSiegeResult() // OK
{
	if(this->m_CastleOccupied == 1 && strcmp(this->m_CastleOwner,"") != 0)
	{
		...
		else
		{
			...
			LogAdd(LOG_BLACK,"[Castle Siege] A guilda %s é a nova proprietária do castelo.",this->m_CastleOwner);
...
void CCastleSiege::CGCastleSiegeNpcUpgradeRecv(PMSG_CASTLE_SIEGE_NPC_UPGRADE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->NpcClass == 277)
	{
		...
		else if(lpMsg->UpgradeType == 3)
		{
			...
			{
				LogAdd(LOG_RED,"[Castle Siege] Erro ao aprimorar o NPC: Life Gate [%d][%d]",lpNpc->LifeLevel,lpMsg->UpgradeValue);
			}
	...
	else if(lpMsg->NpcClass == 283)
	{
		...
		else if(lpMsg->UpgradeType == 3)
		{
			...
			if(next_level != (current_level+1))
			{
				LogAdd(LOG_RED,"[Castle Siege] Erro ao atualizar o NPC: Life Statue [%d][%d]",lpNpc->LifeLevel,lpMsg->UpgradeValue);
			}
