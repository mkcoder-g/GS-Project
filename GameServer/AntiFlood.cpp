		...
		else
		{
			...
			if(it->second.Blocked == true)
			{
				if(elapsedTime >= (DWORD)gServerInfo->m_AntiFloodBlockTime)
				{
					...
					gLog->Output(LOG_HACK,"[AntiFlood] O endereço IP %s foi desbloqueado.",IpAddress);
				}
				else
				{
					return false;
			...
			else
			{
				if(elapsedTime <= (DWORD)gServerInfo->m_AntiFloodMaxTime)
				{
					...
					if(it->second.Count > gServerInfo->m_AntiFloodMaxCount)
					{
						...
						gLog->Output(LOG_HACK,"[AntiFlood] O endereço IP %s foi bloqueado devido a um número excessivo de tentativas.",IpAddress);
						return false;
					}
				}
				else
