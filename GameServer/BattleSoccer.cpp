...
void CBattleSoccer::SetState_EMPTY() // OK
{
	...
	LogAdd(LOG_BLACK,"[Battle Soccer] Definir estado VAZIO");
}
...
void CBattleSoccer::SetState_STAND() // OK
{
	...
	LogAdd(LOG_BLACK,"[Battle Soccer] Definir estado STAND");
}
...
void CBattleSoccer::SetState_START() // OK
{
	...
	LogAdd(LOG_BLACK,"[Battle Soccer] Definir estado INICIAR [%s] vs [%s]",this->m_BattleSoccerInfo[0].Guild->Name,this->m_BattleSoccerInfo[1].Guild->Name);
}
...
void CBattleSoccer::SetState_CLEAN() // OK
{
	...
	LogAdd(LOG_BLACK,"[Battle Soccer] Definir estado como LIMPO");
}
