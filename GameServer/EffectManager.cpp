...
void CEffectManager::MainProc() // OK
{
	for(int n=0;n < MAX_OBJECT;n++)
	{
		...
		for(int i=0;i < MAX_EFFECT_LIST;i++)
		{
			...
			if (lpEffect->m_index == EFFECT_INVISIBILITY)
			{
				LogAdd(LOG_BLUE, "lpEffect->m_count == %d - Effect == %d", lpEffect->m_count, lpEffect->m_index);
			}

			if (lpEffect->m_count <= 1 && lpEffect->m_index == EFFECT_INVISIBILITY)
			{
				if (gEffectManager->CheckEffect(&gObj[n], EFFECT_INVISIBILITY) != 0)
				{
					LogAdd(LOG_BLUE, "EFEITO REMOVIDO");

					gObj[n].IsSkillInvisible = 0;
